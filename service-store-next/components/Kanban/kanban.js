...
export default function Kanban(props) {
  ...
  useEffect(async () => {
    ...
    const responseMessages = await axios.post(
      `http://islab-thesis.aegean.gr:82/trans/api/messages`,
      { id: active_user.id }
    );
    ...
    for (const chat of responseMessages.data) {
      ...
      if (
        ...
        lastMessage.senderId != active_user.id
      ) {
    ...
    response.data.forEach((request) => {
      cards.push({
        ...
        description: request.service_1.substr(
  ...
  const newRating = () => {
    ...
    setRateModalIsOpen(false);
    toast.notify("Η αξιολόγηση υποβλήθηκε επιτυχώς", {
      duration: 5,
      type: "success",
      title: "Service Store",
    });
  ...
  const navigateToAnonymoysChat = () => {
    Router.push({
      ...
      query: {
        ...
        name: activeRequest.fullName,
        created: activeRequest.created,
      },
    });
  };
